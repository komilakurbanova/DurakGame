...
def game_block(update, context: CallbackContext, flag_inline_card: bool) -> None:
    ...
    if p1.username == username and p1.active and p2.defensive:
        ...
        if message == "Подкинуть":
            ...
            # так ведь и было при входе сюда?
            # p1.active = True
            # p1.defensive = False
            # p2.active = False
            # p2.defensive = True
    ...
    if p1.username == username and p1.active and not p1.defensive:
        ...
        if message != "OK":
            if len(p1.attack_hand):
                context.bot.send_message(chat_id=player1.chat_id, text="Вы уже положили карту!",
                                         reply_markup=ReplyKeyboardMarkup(hand_1, one_time_keyboard=True,
                                                                          resize_keyboard=True, ))
                return
            ...
            if not flag_ok:
                ...
                context.bot.send_message(chat_id=player1.chat_id, text="Нажми ОК, чтобы подтвердить",
                                         reply_markup=ReplyKeyboardMarkup(hand_1, one_time_keyboard=True,
                                                                          resize_keyboard=True, ))
        else:
            ...
            context.bot.send_message(chat_id=player2.chat_id,
                                     text='Нажмите на карту, которую хотите отбить.', reply_markup=kb)
